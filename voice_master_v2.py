...
# ==========================================
# ⚠️ Insert your real OpenRouter API Key here!
OPENROUTER_API_KEY = "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"
...
client = OpenAI(
    ...
    default_headers={"X-OpenRouter-Title": "Stretch-Voice-Master"},
)
...
def parse_voice_intent(user_command: str) -> dict:
    ...
    try:
        response = client.chat.completions.create(
            model="openai/gpt-4o-mini",  # 如果你有 gpt-4o 的权限，可以换成 openai/gpt-4o
            messages=[
...
def execute_task1():
    ...
    os.system('wsl.exe -d Ubuntu bash -c "pkill -9 -f rerun"')
...
def execute_task2_action(intent: str, value: float):
    ...
    cmd = (
        ...
        f'python task2_client.py --action {intent} --val {value} --robot_ip 172.22.243.38"'
    )
    os.system(cmd)
...
# ==========================================
def record_and_transcribe(recognizer, source, audio_model, phrase_time_limit):
...
def main():
    ...
    print("   [SPACE] : Hold/Press to Speak")
    ...
    while True:
        ...
        if keyboard.is_pressed('1'):
            ...
            task2_loop(recognizer, audio_model)
            continue
        ...
        # --- Push to Talk (Spacebar) ---
            ...
            with sr.Microphone() as source:
                text = record_and_transcribe(recognizer, source, audio_model, phrase_time_limit=8.0)
...
def task2_loop(recognizer, audio_model):
    print("\n" + "=" * 45)
    ...
    while True:
        ...
        # --- Push to Talk for Movement ---
        elif keyboard.is_pressed('space'):
            while keyboard.is_pressed('space'): time.sleep(0.01)

            with sr.Microphone() as source:
                text = record_and_transcribe(recognizer, source, audio_model, phrase_time_limit=6.0)
